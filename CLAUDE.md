...
`luminality-ui` is currently consumed by `luminality-web` in the luminalityai workspace, via the `@luminalityai/ui` public npm package.

`fundbright-web` and `nutripod-web` do not consume this package — they have their own design systems.

This package is published to npm (registry.npmjs.org), not RubyGems — the workspace-level `/rollout-gem` skill (which targets rubygems consumers) does NOT apply here. To roll out a new version, bump the `@luminalityai/ui` version in `luminality-web/package.json` and run `npm install`.
