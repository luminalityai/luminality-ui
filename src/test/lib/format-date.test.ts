...
describe("format-date utilities", () => {
  ...
  describe("formatDateTime", () => {
    ...
    it("formats datetime in specific timezone", () => {
      expect(formatDateTime(testDate, false, "America/New_York")).toBe(
        "2024.01.01 07:00",
      )
      expect(formatDateTime(testDate, false, "Europe/Paris")).toBe(
        "2024.01.01 13:00",
      )
    })
  ...
  describe("formatDateTimeWithTimezone", () => {
    it("includes timezone abbreviation", () => {
      expect(formatDateTimeWithTimezone(testDate, "UTC")).toBe(
