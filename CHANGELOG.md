...
- **Animation utilities now come from `tw-animate-css` instead of `tailwindcss-animate`.** `tailwindcss-animate` is a Tailwind v3 plugin; `tw-animate-css` (1.4.0) is its Tailwind v4 CSS-first successor, loaded with `@import "tw-animate-css"` and using the same class names. This matches luminality-web (luminality-web#1255). Every animation class the components use is still in `@luminalityai/ui/styles`, and the keyframe set is unchanged (`enter`, `exit`, `fade-in`, `accordion-down`, `accordion-up`). The stylesheet grows from 33.9 kB to 36.6 kB (6.9 kB to 7.2 kB gzip). `tw-animate-css` is a devDependency, because it is compiled into the stylesheet at build time, so consumers no longer install `tailwindcss-animate`.
  - **The Accordion now animates.** Its `data-[state=open]:animate-accordion-down` and `data-[state=closed]:animate-accordion-up` were never generated before, because the hand-written `.animate-accordion-*` rules were not Tailwind utilities and so had no variants. `tw-animate-css` provides them, and the hand-written accordion keyframes are removed. The bare, unprefixed `.animate-accordion-down` and `.animate-accordion-up` rules are no longer in the stylesheet. No component used them.
  - **The stylesheet honours `prefers-reduced-motion: reduce`.** It pins `--tw-animation-duration` to `0.01ms`, so overlay enter/exit and accordion animations jump straight to their final frame, and it turns off `animate-fade-in`. The duration is 0.01ms rather than 0 so `animationend` still fires, because Radix waits for that event before it unmounts a closing overlay. `animate-spin` and `animate-pulse` are left alone. This is the same rule luminality-web uses.
  - **The repo's Markdown docs are no longer scanned for classes.** `src/styles/index.css` adds `@source not "../../*.md"`, alongside the existing `design/` exclusion, so class names written as prose in `CHANGELOG.md` or `README.md` do not end up in the published stylesheet. Before this entry, the docs did not add any classes.
